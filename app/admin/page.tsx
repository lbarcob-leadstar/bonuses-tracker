...
export default function AdminPage() {
  ...
  const toggleActive = async (casino: Casino) => {
    await supabase
      .from('casinos')
      .update({ is_active: !casino.is_active })
      .eq('id', casino.id)
    setCasinos((prev) =>
      prev.map((c) => c.id === casino.id ? { ...c, is_active: !c.is_active } : c)
    )
  }
          ...
          <div className="rounded-2xl p-6 mb-6"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <input
                ...
                value={newForm.bonus_description}
        ...
        <div className="space-y-3">
          {casinos.map((casino) => (
            <div key={casino.id}
              ...
              style={{
                background: '#2C343F',
                border: `1px solid ${casino.is_active ? 'rgba(255,255,255,0.08)' : 'rgba(229,45,75,0.2)'}`,
                opacity: casino.is_active ? 1 : 0.5,
              }}>
              {editingId === casino.id ? (
                <div className="flex flex-col md:flex-row gap-3">
                  ...
                  />
                  <input
                    value={editForm.bonus_description}
                    onChange={(e) => setEditForm((p) => ({ ...p, bonus_description: e.target.value }))}
                    className="flex-[2] px-3 py-2 rounded-xl text-sm outline-none"
                    style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,231,153,0.4)', color: '#f0f0f0' }}
                  />
                  <div className="flex gap-2">
                    <button onClick={() => saveEdit(casino.id)}
                      className="px-4 py-2 rounded-xl text-sm font-bold cursor-pointer"
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => toggleActive(casino)}
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors"
                      style={{
                        background: casino.is_active ? 'rgba(255,255,255,0.05)' : 'rgba(229,45,75,0.2)',
                        color: casino.is_active ? 'rgba(255,255,255,0.4)' : '#E52D4B',
                        border: '1px solid rgba(255,255,255,0.1)',
                      }}>
                      {casino.is_active ? 'Hide' : 'Show'}
                    </button>
                    <button onClick={() => startEdit(casino)}
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer"
                      style={{ background: 'rgba(255,231,153,0.1)', color: '#FFE799', border: '1px solid rgba(255,231,153,0.2)' }}>
                      Edit
                    </button>
                    <button onClick={() => deleteCasino(casino.id)}
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer"
                      style={{ background: 'rgba(229,45,75,0.1)', color: '#E52D4B', border: '1px solid rgba(229,45,75,0.2)' }}>
                      Delete
                    </button>
                  </div>
                </div>
