...
export default function AppPage() {
  ...
  const toggleClaim = async (casino: CasinoWithClaim) => {
    ...
    const optimistic = casinos.map((c) =>
      c.id === casino.id ? { ...c, claimed_today: !c.claimed_today } : c
    )
    setCasinos(optimistic)

    if (!casino.claimed_today) {
      // Claim: calculate streak
      const { data: lastClaim } = await supabase
        .from('user_claims')
        .select('claimed_date, streak')
        .eq('user_id', user.id)
      ...
      const newStreak =
        lastClaim?.claimed_date === yesterdayStr ? (lastClaim.streak ?? 0) + 1 : 1
      ...
      setCasinos((prev) =>
        prev.map((c) => c.id === casino.id ? { ...c, claimed_today: true, streak: newStreak } : c)
      )
    } else {
      // Unclaim
      await supabase
      ...
      setCasinos((prev) =>
        prev.map((c) => c.id === casino.id ? { ...c, claimed_today: false, streak: 0 } : c)
      )
    }
  }
    ...
    <div className="min-h-screen" style={{ background: '#1e252e' }}>
      ...
      <header style={{ background: '#2C343F', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-black text-lg leading-none" style={{ color: '#FFE799' }}>
                Bonus Tracker
              </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="rounded-2xl p-6 mb-8"
          style={{ background: '#2C343F', border: '1px solid rgba(255,255,255,0.1)' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-right">
              <p className="text-4xl font-black" style={{ color: '#E52D4B' }}>
                {Math.round(progress)}%
              </p>
          ...
          <div className="w-full h-3 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}>
            <div
              ...
              style={{
                width: `${progress}%`,
                background: 'linear-gradient(90deg, #E52D4B, #FFE799)',
                boxShadow: '0 0 10px rgba(229,45,75,0.5)',
              }}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((casino) => (
            <div
              ...
              style={{
                background: casino.claimed_today
                  ? 'rgba(229,45,75,0.1)'
                  : '#2C343F',
